...
`#38`'s real entry ranks **8th** out of 677 chunks. With BM25, the
...
ranked `8th` in this chapter's "Why not dense embeddings?" section),
...
- For this archive's dense technical phrasing, keyword search (BM25)
  ...
  level shows up again at the sentence level. Four test queries is a
  real result, not a toy one, but it's still too small to call this a
  statistically powered comparison — read it as consistent with BM25's
  known strength on exact-jargon matching, not as proof dense
  embeddings lose on this archive in general.
