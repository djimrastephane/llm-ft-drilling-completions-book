...
teaching choice, not a tuned production schedule. It also trains on
`new_examples` alone, with none of `current_examples` mixed back in —
a real instance of what the field calls **catastrophic forgetting** (or,
more precisely here, recency bias): training only on new data can pull
a model's behavior away from what it learned earlier, even without
erasing it outright. The standard mitigation — replaying a sample of
`current_examples` alongside each new batch — isn't attempted here;
Step 3 and the Field Notes below show part of what skipping it costs.
...
across this book's own checkpoints -- a similar "latest doesn't mean
best" regression, found separately by comparing the book's own
persisted Chapter 5/8/13 checkpoints (this one regresses on both
average overlap and perplexity, not the single metric this chapter's
own version-comparison run above flagged -- a different checkpoint
pair, not the same result), surfaced automatically by the
