...
::: {.callout-important title="This is a teaching tool, not a safety check"}
`faithfulness_score` exists so you can see, in code you can read start
to finish, what "grounded" does and doesn't guarantee — it is not
validated for deciding whether an automated answer is safe to act on. A
field engineer relying on a model-generated summary for a shift
handover cannot treat a high score here as a stand-in for reading the
source report. Field Notes below is the proof: a genuinely dangerous
reversal — the report said *trip in hole*, the answer said *trip out of
hole* — still scored `0.80`, comfortably above this chapter's own `0.5`
threshold. Use this score as a prompt for a qualified person to go
check the source, never as an unsupervised pass/fail on a live well
operation.
:::
...
work, only what happens to the result afterward. Note the naming shift:
Chapter 9's `build_grounded_prompt` just means "built from retrieved
text," true whether or not the model actually used it; this step's own
`"grounded"` field is the stricter claim — true only once at least one
source has passed the faithfulness check below. Same word, two
different chapters, deliberately different strictness; watch for which
one is being made wherever you see it. `threshold=0.5` is a plain
default, not tuned to any one answer; Step 3 shows what it does
...
- This chapter's faithfulness score is an educational baseline, not a
  validated safety system — report `#49`'s reversed-direction answer
  scored `0.80` all the same. It must never be used as an unsupervised
  safety check on live well operations; a qualified person checking the
  actual source stays the real safeguard, especially for a handover
  summary someone might act on.
