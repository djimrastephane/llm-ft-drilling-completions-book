...
Farid, the production engineer, pushes back on Chapter 1's finding:
...
A real instruction/response pair, built the same way Chapter 2's
`Drilling_038` examples were, but this one drawn from a genuine
TIME BREAKDOWN entry in `Drilling_017` (2020-11-05), quoted exactly as
filed:

::: {.callout-note title="Sample training example -- FORGE-16A-78-32_Drilling_017_2020-11-05.pdf"}
```json
{
  "instruction": "Summarize what operation this report excerpt describes.",
  "input": "TOOH (trip out of hole) with BHA #2. Break off bit, lay down Scout RSS (rotary steerable).",
  "output": "A planned bottom hole assembly change: BHA #2 is pulled out of hole, its bit is broken off, and the rotary steerable tool is laid down ahead of picking up the next BHA."
}
```
:::

`TOOH` and `BHA` sit right in the `input` above. Before this chapter is
...
Farid's instinct — that a language model trained on huge amounts of text
...
open `datasets/full_training_set/FORGE-16A-78-32_Drilling_017_2020-11-05.pdf`
and pick two more real abbreviations from its TIME BREAKDOWN table
(`RSS` and `TIH` both appear, each self-glossed in parentheses right in
the report, the same way `TOOH` is above) and check how they tokenize.
...
across the 9 drilling reports (`36` and `32` times respectively, on a
real count). A reference solution is at
`code/chapter_04/challenge/challenge.py` — on a real run, `TVD` vs.
`"true vertical depth"` scores `0.024` raw / `0.118` sentence-embedding,
in the same range as this chapter's other acronym pairs, and the same
run's `count_term_occurrences()` reproduces both counts above (and the
Field Notes' `36` for `BOP`) directly from the archive.
...
embedding model would fix it -- though, as you'll see there, Chapter
5's fine-tune leaves this exact raw embedding table frozen and untouched;
the fix happens at the attention layers downstream of it instead.
:::
