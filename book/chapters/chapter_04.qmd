...
well-control safety device, appearing 38 times across this book's 9
...
**Why:** neither embedding approach has ever seen enough of this
archive's specific usage to anchor `BOP` to its real meaning. The
sentence-embedding model's `0.320` for `"birthday party"` almost
certainly picks up on `B`/`P`-sounding surface similarity
(`BOP`/`birthday party`) rather than anything about well control — a
coincidence, not a judgment, but a costly one to trust blindly.
