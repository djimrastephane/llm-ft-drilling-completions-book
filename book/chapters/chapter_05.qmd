...
Carlos, the completions engineer, has watched this book build up evidence
...
training -- including the raw embedding table Chapter 4 measured;
`BHA`'s own coordinate doesn't move. What changes instead is how the
attention layers *use* that unchanged coordinate once text passes
through them, which is where this chapter's improvement actually comes
from.
...
Carlos's question with numbers: did the model learn these exact examples,
