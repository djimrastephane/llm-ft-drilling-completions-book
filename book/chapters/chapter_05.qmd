...
shape as the layer it's correcting, without ever storing that full-size
version directly. How narrow those two tables are is the adapter's
**rank** (`r=8` in Step 2 below) — small by design. Fewer numbers to
store and train, at the cost of only approximating what a full-size
correction could do.
:::

::: {.content-visible when-format="html"}
::: {.pipeline-diagram}
![](../figures/diagrams/lora_lowrank_ch05_light.svg){.diagram-light width="480" fig-alt="Two-row diagram. Row 1: a tall Table A multiplied by a wide Table B equals a full-size correction grid, computed rather than stored. Row 2: the frozen base weights plus that correction equal what the model reads at inference."}
![](../figures/diagrams/lora_lowrank_ch05_dark.svg){.diagram-dark width="480"}

*Figure: LoRA's correction is built from two small tables, then added on top of the frozen base weights — it never replaces them.*
:::
:::

::: {.content-visible when-format="pdf"}
![LoRA's correction is built from two small tables, then added on top of the frozen base weights -- it never replaces them.](../figures/diagrams/lora_lowrank_ch05.pdf){width="480" fig-align="center" #fig-lora-lowrank-pdf}
:::
