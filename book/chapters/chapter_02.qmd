...
### Step 3: Build and save the full training set
...
re-running PDF extraction every time.
...
book uses for training data) plus a summary line reporting how many
examples were built and how many reports were skipped.
...
Built 18 training examples -> datasets/training_examples/sample_training_examples.jsonl
```

## What just happened?

Nine of the ten sample reports use the drilling-report field layout, so
each contributes two examples — 18 total. The tenth,
`Completion_003`, is a *completion* report using different field labels
(`Present Ops:` / `Next 24 Hours:` instead of `PRESENT OPERATIONS:` /
`ACTIVITY PLANNED:`) and gets correctly skipped rather than silently
producing a garbage example. `datasets/training_examples/` is generated,
not committed to this repository — regenerate it any time by re-running
this script (see `book/.gitignore`).
...
**You'll know it worked when:** the script prints `Built 18 training
examples`, the file has exactly 18 lines, and the first one matches
report `#3` (`Drilling_003`, 2020-10-22) — the earliest report in the
sample set.
...
Hours:`, and a two-digit-year date format), so all 10 sample reports
contribute examples instead of 9. A reference solution is at
`code/chapter_02/challenge/challenge.py` — it produces 20 training
...
**`build_training_examples.py`** — turns the 9 drilling-format reports
in `datasets/sample_training_set/` into 18 real training examples,
saved to `datasets/training_examples/sample_training_examples.jsonl`.
Chapter 5 fine-tunes on data built exactly this way, at a larger scale.
:::
...
against a batch of real questions drawn from the training examples you
just built — so you have hard, reproducible evidence of exactly what it
gets wrong, and something concrete to compare Chapter 5's fine-tuned
model against afterward.
