...
Every operator has its own language on the rig. Morning report
shorthand, internal asset codes, and practical field judgment calls
rarely show up in public data. When you ask a general-purpose AI a
complex drilling or completions question, the result is often polished
and plausible, yet subtly wrong: the right textbook terms, but the
wrong wellsite logic.

Fine-tuning fixes this, but it cannot be treated as a black box.
Success requires high-quality operational data, the right training
strategy, and clear validation metrics to prove the model genuinely
improved rather than just shifted its phrasing.
...
One expectation worth setting early: this book's own code is written
and tested against one real report format (Utah FORGE's Daily Drilling
Report). There is no single format across operators, contractors, and
basins, so the goal here isn't to hand you a tool that already expects
your archive's layout — it's to teach you the pipeline stage by stage,
on real data, well enough that adapting each stage to your own reports
is a rewrite you understand, not a mystery you're stuck on. See
[Appendix C](appendix/appendix_c_known_limitations.qmd) once you've
finished the book for exactly where that adaptation work starts.
...
- **Known limits.** Fine-tuning teaches style and domain patterns; it is
  ...
  current document verbatim — Chapter 9 covers combining the two. See
  [Appendix C](appendix/appendix_c_known_limitations.qmd) for six
  concrete gaps — from brittle report parsing to a faithfulness check
  that misses a reversed operation — worth checking before you point
  this pipeline at your own archive. That faithfulness check in
  particular is a teaching tool, not a validated safety system — never
  use it, unsupervised, as the check on whether a live-well handover
  summary is safe to act on.
...
| Carlos | Completions Engineer |
| Sarah | Intervention Engineer |
| Farid | Production Engineer |

They're a framing device, not a source of data: whichever question Oumy,
Carlos, Sarah, or Farid asks, every number and result used to answer it must
